...
const Adjust = (props) => {
  ...
  const prevStatusRef = useRef(status); // Define useRef at the top level

  useEffect(() => {
    console.log(status);
    // Map "connecting ..." to "idle" for the purpose of this logic
    const effectiveStatus = status === "connecting ..." ? "idle" : status;
    ...
    if (prevEffectiveStatus !== "idle" && effectiveStatus === "idle") {
