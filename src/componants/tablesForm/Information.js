...
const Information = (props) => {
  const dispatch = useDispatch();
  const ref3 = useRef();
  const appDataReloadKey = useSelector((state) => state.general.appDataReloadKey);
  const [positiveDesignLoad, setPositiveDesignLoad] = useState(0);
  const [negativeDesignLoad, setNegativeDesignLoad] = useState(0);
  const [projectName, setProjectName] = useState("");
  const [isEditable, setIsEditable] = useState(false);
      ...
      axios
        ...
        .catch((error) => {
          ...
          if (error.response && error.response.status === 400) {
            const errorDetail = error.response.data?.detail || "An error occurred while creating the project";
            setProjectError(errorDetail);
          } else {
            setProjectError("An unexpected error occurred while creating the project");
          }
        });
    }
  };
    ...
    if (newParent.trim() !== "") {
      axios
        ...
        .catch((error) => {
          ...
          // Extract error details from response
          if (error.response && error.response.status === 400) {
            const errorDetail = error.response.data?.detail || "An error occurred while creating the parent";
            setParentError(errorDetail);
          } else {
            setParentError("An unexpected error occurred while creating the parent");
          }
        });
    }
  };
  ...
  // Soft-reload projects/parents when Sync returns 0 (no page refresh)
  useEffect(() => {
    if (appDataReloadKey === 0) {
      return;
    }
